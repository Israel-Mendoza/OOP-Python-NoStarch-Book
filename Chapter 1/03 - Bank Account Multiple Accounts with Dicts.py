...
def get_account_number() -> int:
    while True:
        ...
        if account_number_int < 0:
            ...
            continue
        if (account_number_int) < len(accounts_list):
...
def check_password(account_index: int) -> bool:
    ...
    if entered_password != accounts_list[account_index]["password"]:
...
def create_new_account() -> None:
    ...
    while True:
        ...
        if len(password) < 3:
            ...
            break
    accounts_list.append({"name": name, "password": password, "balance": 100})
    print(f"Your account number is '{len(accounts_list) - 1}'")
...
def get_balance(account_index: int) -> None:
    ...
    if check_password(account_index):
        ...
        print(
            f"The current balance in your account is ${accounts_list[account_index]['balance']}.00"
        )
...
def make_deposit(account_index: int) -> None:
    ...
    if check_password(account_index):
        ...
        simulate_processing_transaction()
        accounts_list[account_index]["balance"] = (
            int(accounts_list[account_index]["balance"]) + deposit_amount_int
        )
        print(f"Final balance: ${accounts_list[account_index]['balance']}.00")
...
def withdrawal(account_index: int) -> None:
    ...
    if check_password(account_index):
        if withdraw_amount_int <= int(accounts_list[account_index]["balance"]):
            print(f"Withdrawing ${withdraw_amount_int}.00 from your account...")
            simulate_processing_transaction()
            accounts_list[account_index]["balance"] = (
                int(accounts_list[account_index]["balance"]) - withdraw_amount_int
            )
            print(f"Final balance: ${accounts_list[account_index]['balance']}.00")
        else:
            print(f"Withdrawing ${withdraw_amount_int}.00 from your account...")
            ...
            print(f"Current balance: {accounts_list[account_index]['balance']}")
...
def show_account(account_index: int) -> None:
    ...
    if check_password(account_index):
        ...
        simulate_processing_transaction()
        print(f"Account name: {accounts_list[account_index]['name']}")
        print(f"Account password: {accounts_list[account_index]['password']}")
        print(f"Account balance: ${accounts_list[account_index]['balance']}.00")
...
def show_all_accounts() -> None:
    if len(accounts_list) > 0:
        for i in range(len(accounts_list)):
            print(
                f"Account #{i} - {accounts_list[i]['name']}. Balance: ${accounts_list[i]['balance']}.00"
            )
